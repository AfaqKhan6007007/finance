...
def test_case_1_missing_required_fields():
    ...
    if "country" in response.lower() and ("missing" in response.lower() or "need" in response.lower() or "provide" in response.lower()):
        print("\n[OK] PASS: Chatbot correctly identified missing 'country' field")
    else:
        print("\n[FAIL] FAIL: Chatbot did not ask for missing country field")
...
def test_case_2_provide_missing_field(session):
    ...
    if "created" in response.lower() or "success" in response.lower():
        print("\n[OK] PASS: Company created successfully")
    else:
        print("\n[WARNING] WARNING: Creation status unclear")
...
def test_case_3_create_with_fk():
    ...
    if has_company_names and has_selection_prompt:
        print("\n[OK] PASS: Chatbot listed companies and asked user to select")
        has_company_list = True
    else:
        print("\n[FAIL] FAIL: Chatbot did not properly list companies for selection")
...
def test_case_4_select_fk_option(session):
    ...
    if "created" in response.lower() or "success" in response.lower():
        print("\n[OK] PASS: Account created successfully with FK link")
    else:
        print("\n[WARNING] WARNING: Creation status unclear")
...
def test_case_5_complete_info_upfront():
    ...
    if ("created" in response.lower() or "success" in response.lower()) and "missing" not in response.lower():
        print("\n[OK] PASS: Company created directly with all info provided")
    else:
        print("\n[WARNING] WARNING: May have asked for additional info unnecessarily")
...
def main():
    ...
    try:
        ...
        print("  [OK] Chatbot asks for missing required fields")
        print("  [OK] Chatbot lists FK options when needed")
        print("  [OK] Chatbot accepts user selections and creates records")
        print("  [OK] Chatbot creates directly when all info provided")
