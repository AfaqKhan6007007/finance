"""
Simple schema guide verification - directly analyze the prompt_tools.py file
"""
...
def extract_schema_guide(file_content, function_name):
    """Extract schema guide text from the file"""
...
def analyze_schema_guide(schema_text, model_name, model_class):
    ...
    if not schema_text:
        return {'model': model_name, 'error': 'Schema guide not found'}
    ...
    # Count FK mentions
    actual_fks = [f.name for f in model_class._meta.fields if f.is_relation]
    results['fk_count_mentioned'] = sum(1 for fk in actual_fks if fk in schema_text)
    
    # Count choice fields mentioned
    actual_choices = [f.name for f in model_class._meta.fields if f.choices]
...
def main():
    ...
    # Read the prompt_tools.py file
    with open('mcp_server/tools/prompt_tools.py', 'r', encoding='utf-8') as f:
        file_content = f.read()
    ...
    for model_class, method_name, table_name in models_and_guides:
        # Extract schema guide text
        schema_text = extract_schema_guide(file_content, method_name)
        ...
        if schema_text:
            ...
            if results['overall_score'] >= 90:
                print("  Status: EXCELLENT [OK]")
            elif results['overall_score'] >= 70:
                print("  Status: GOOD [OK]")
            elif results['overall_score'] >= 50:
                print("  Status: ADEQUATE [WARNING]")
            else:
                print("  Status: NEEDS IMPROVEMENT [ERROR]")
        else:
            print(f"\n[{table_name.upper()}]")
            print(f"  ERROR: Schema guide function not found!")
            all_results.append({'model': table_name, 'error': 'Not found'})
    ...
    if valid_results:
        ...
        if issues:
            ...
            print("\nNo significant issues found! All schema guides are comprehensive. [OK]")
